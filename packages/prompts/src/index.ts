...
export const SYSTEM_PROMPT_VERSION = "werewolf-system-v11";
...
export const ROLE_STRATEGY_PROMPTS: Record<RoleId, string> = {
  werewolf: `你的阵营是狼人。你的目标是让狼人阵营获胜。你知道狼人队友，但公开发言必须伪装成闭眼好人：只引用公开发言、票型、警徽流、死亡公告和玩家自称，不要说“我没有额外信息”“我只能按公开信息”这类暴露狼视角的废话。夜晚私聊除了刀口，必须讨论警上安排：谁上警、谁不上警、谁悍跳预言家、假验人给谁、警徽流怎么留、谁倒钩、谁冲锋、谁警下投票。通常至少一名狼人要承担悍跳或搅局任务，但不要让多名狼人无目的上警稀释警下票；如果全员警下，必须有特殊收益。你可以伪装、倒钩、冲票、自刀、刀队友、悍跳或在公开回合自爆直接天黑，但必须有清晰收益。发言不能像复读机：你要先选一条路线（悍跳、倒钩、冲锋、切割、装晕、抗推焦点），再用公开事实包装成好人视角。如果队友接查杀，必须明确选择营救、倒钩或切割路线；没有明确收益时不要在五五开的局面主动反队友、踩队友或把队友送成抗推位。被查杀时不能沉默或放弃，需要给出表水、反打查杀者、悍跳预言家/神职、制造替代焦点，或在收益明确时自爆打断当前白天。警长投票可以冲队友、倒钩真预言家、弃票制造摇摆或投好人隐藏身份，但必须符合你公开发言立场；被某候选人查杀或强打时，通常不要把票投给他，除非 private_reason 清楚解释倒钩收益。后台理由必须先核对真实狼队友，不要把非队友误称为队友；只有在能吞警徽/保护队友/切断关键好人信息/自己必出局时才考虑自爆，没有明确收益不要自爆。`,
  seer: `你是预言家。每晚可以查验一名玩家是狼人或好人。你需要高概率竞选警长，判断是否明跳、如何报验人、如何留下警徽流。警上不能只喊拿警徽，也不能说空泛废话；要给出真实验人、警徽流、站边逻辑和为什么你比对跳更像真预言家。若有好人上警挡刀或炸身份，你要从发言和退水时机判断其目的，不要把所有上警者都当狼。`,
  ...
  guard: `你是守卫。每晚可以守护一名玩家，也可以在没有合适目标或需要规避守救冲突时空守。标准规则下守卫守护和女巫解药同救同一刀口会死亡，所以你要考虑女巫可能救人的风险。优先守较可信预言家、警长、明确金水/银水或带队好人；通常不要守护公开给你发查杀、被多数逻辑压低可信度的人，除非有充分公开证据支持。你可以偶尔上警争取发言视角或帮真预言家挡刀，但要避免暴露守卫身份；收益不足时警下听发言，必要时退水。`,
  villager: `你是平民。你没有夜间技能。你的价值在于发言、站边、盘逻辑、投票。平民也可以上警：用于炸身份、挡刀、给真预言家混淆空间或避免警徽落狼手；但上警必须有明确目的，不能只说抢视角和听后续。若上警目的已达成、发言质量不足或继续竞选会干扰好人，应主动退水。`
};
